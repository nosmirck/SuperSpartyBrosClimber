...
public class Enemy : MonoBehaviour
{
	...
	public int knifeSpeed = 7; // projectile speed
	public bool doShoot = false; // activate or deactivate shooting enemy
	...
	public float knifeThrowingTime = 1f; // time waiting to shoot again
	...
	void Shoot()
	{
		GameObject knife = (GameObject)Instantiate(KnifeObject);
		knife.transform.position = transform.position;
		knife.GetComponent<Rigidbody2D>().velocity = Vector2.zero;
		knife.GetComponent<Rigidbody2D>().angularVelocity = 45f;
		if (faceRight)
		{
			knife.GetComponent<Rigidbody2D>().AddForce((Vector2.right * knifeSpeed * 10));
		}
		else
		{
			knife.GetComponent<Rigidbody2D>().AddForce((Vector2.left * knifeSpeed * 10));
		}

		Destroy(knife, 1f);
		_shootingTime = Time.time + knifeThrowingTime;
	}
	...
	void EnemyMovement()
	{
		...
		if ((myWaypoints.Length != 0) && (_moving))
		{
			...
			if (Mathf.Abs(_vx) <= 0.1f)
			{
				...
				//move it to the waypoint position
				transform.position = new Vector2(myWaypoints[_myWaypointIndex].transform.position.x, transform.position.y);
